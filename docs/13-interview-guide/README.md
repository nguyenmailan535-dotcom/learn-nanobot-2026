...
> 本节已按 **2026-09-24 HKUDS/nanobot main** 更新。不要再背旧版“4000 行、MEMORY.md + HISTORY.md、固定 Semaphore(3)、Subagent 固定 15 次、save_memory 虚拟工具”等答案。

### Q16. Nanobot current-source 的定位是什么？

Nanobot 已经不是早期“几千行教学框架”的形态，而是一套 self-hosted Agent Runtime。它提供 CLI/TUI/WebUI/Gateway/OpenAI-compatible API/Python SDK，多 Provider、多 Channel、Native/MCP Tools、Session/Memory、Subagent、Cron、Heartbeat、Agent Plugins 和安全边界。
...
### Q17. AgentLoop 的 current 运行流程是什么？

AgentLoop 面向一个 Session-facing Turn。
...
其中 run 阶段才进入 AgentRunner。

### Q18. AgentRunner 与 AgentLoop 为什么拆开？

- AgentLoop 关心 Channel、Session、Workspace、Command、Delivery、Recovery。
- AgentRunner 关心 Provider、Tool Call、Streaming、Compaction、Tool Result、Iteration。

两者变化原因不同。拆开后 Subagent 等场景可以复用 Runner，不需要复制整个聊天产品层。

### Q19. ContextBuilder 做什么？

ContextBuilder 决定“这一次模型真正看到什么”。
...
重点：Session 保存什么 ≠ Context 本轮放什么。

### Q20. current Memory System 怎么设计？
...
### Q21. AutoCompact 与 Dream 有什么区别？

AutoCompact 主要解决 Session Context 太长，偏 Context Management。

Dream 主要从长期 history archive 中整理 Durable Memory，偏 Long-term Memory Curation。

### Q22. ToolRegistry 的职责是什么？

统一管理 model-callable Tool：
...
AgentRunner 不需要区分 Native Tool、MCP Tool 或 Plugin Tool。

### Q23. ToolLoader 与 ToolRegistry 的区别？

ToolLoader 负责“发现/构造哪些 Tool 应进入 Registry”。

ToolRegistry 负责“运行时如何统一暴露和执行这些 Tool”。

可以类比 Spring 的 Bean discovery/registration，但 Tool 还包含 model-facing schema。

### Q24. Nanobot current MCP 如何集成？

应用组合层创建 shared ToolRegistry 和 MCPProvider。
...
MCPProvider 管 connect/reconnect/close，AgentLoop 不拥有 MCP Connection lifecycle。

### Q25. enabledTools 的作用是什么？

它是 MCP 最小权限边界。

Server 可能支持 20 个 Tool，但当前 Agent 只需要 3 个，就只暴露 3 个。减少攻击面，也减少模型 Tool Selection 空间。

### Q26. current Session 并发怎么控制？

同 Session 使用 pending queue + single worker 保证 FIFO，并配合 Session Lock。

全局 Inbound Request 并发由环境变量控制：

~~~text
NANOBOT_MAX_CONCURRENT_REQUESTS
~~~

未设置、0 或负数表示 Nanobot 不主动加 global cap。

### Q27. Subagent current 并发怎么控制？
...
它和 Inbound Request concurrency 是两套资源控制。

### Q28. Subagent 还固定 15 次迭代吗？

不应这样回答。

current SubagentManager 接收 max_iterations，AgentLoop 会同步 runtime limit。资源治理要综合考虑：
...
### Q29. current CronService 还是 APScheduler 吗？

不是旧版示例。

current CronService 自己维护 Job Store/Timer，并使用 croniter 计算 Cron Expression 下一次运行时间。
...
### Q30. Heartbeat current 是什么？

独立 Heartbeat Service 已移除。

Gateway 启动时注册 Protected Heartbeat Cron Job，周期读取：

~~~text
<workspace>/HEARTBEAT.md
~~~

Routine Result 可以静默，只有值得通知的结果才发给用户。

### Q31. Channel 架构如何设计？

Channel 将 Platform-specific Event 转成 InboundMessage，并把 OutboundMessage/Typed Event 投影回平台。

current Channel 是 self-contained package：

~~~text
nanobot/channels/<channel>/
~~~

ChannelManager 负责 discovery、lifecycle、routing、retry。

### Q32. Feishu current 还需要 ngrok 吗？

默认不需要。

current Feishu 使用 WebSocket Long Connection，支持 QR Login 或 App ID/App Secret。只有真正使用 Webhook/Public Callback 的平台才需要公网 HTTPS。

### Q33. Workspace 与 Project Workspace 怎么区分？
...
这允许同一个 Agent Profile 操作多个 Project。

### Q34. current Security 最重要的边界有哪些？
...
- MCP enabledTools
- Session disabled tools
- Plugin manifest/containment/fingerprint

核心原则：Prompt 不是安全边界，Host Permission 才是。

### Q35. 学 Nanobot 最值得面试讲的内容是什么？

不要说“我背过 4000 行源码”。

建议：

> 我沿一条真实消息追过 Channel → MessageBus → per-session FIFO → TurnContext → AgentLoop 七阶段 → AgentRunSpec → AgentRunner → ToolRegistry → Session Persistence；另外做过 MCP、Dream、Subagent 和 Security 的最小实验，能够解释每层输入、输出、状态与 Failure Path。
...
### Q86. TurnContext 为什么存在？

一次 Turn 有 msg、session、runtime、history、request context、provider state、pending queue、tools、usage、delivery 等大量共享状态。

如果每个 Stage 都传几十个参数，会导致 API 混乱。TurnContext 是典型 Context Object，把一次 Turn 的可变执行状态集中起来。

### Q87. current 七阶段 Pipeline 有什么价值？
...
1. 明确 Failure Boundary；
2. 每阶段可以独立 timing；
3. Command 可提前结束，不浪费 LLM Call；
4. Build/Run/Persist/Delivery 职责不混在一起；
5. 更容易测试。

### Q88. per-session pending queue 为什么比“只加一个 Lock”更复杂？

因为用户可能在一个长 Turn 未结束时发送 follow-up。

系统要区分：

- 可 mid-turn injection 的输入
- 必须独立排队的 Turn
- Priority Command
- Automation Turn
- Recovery Turn

单纯 Lock 只能“互斥”，不能表达这些 admission 语义。

### Q89. AgentRunSpec 的设计价值是什么？

它是 Loop → Runner 的 Run Contract，集中描述：
...
避免 Runner 到处读取全局变量，也更容易做 isolated test。

### Q90. Provider Conversation State 是什么？

某些 Provider 能维护/恢复特定会话状态。Nanobot 可以把 Provider-specific state 持久化到 Session，并在 compatible runtime 下恢复；不兼容时回退到 transcript replay。

价值是降低重复上下文传输，同时保留可恢复性。

### Q91. 为什么 Tool Result 要治理大小？

Tool Result 可能：

- 吞掉 Context Window
- 增加 Token/Latency
- 导致 Provider Request 失败
- 引入大量无关内容

current-source 使用 max_tool_result_chars 等机制治理，而不是把一个 magic constant 当永久架构。

### Q92. concurrent_tools=true 有什么风险？

一次模型响应中多个 Tool Call 可并发提高 I/O 吞吐，但如果 Tool 有写副作用：

- 执行顺序不确定
- 共享文件/DB 竞争
- 相互依赖结果失效

所以 Tool Schema/Prompt 应鼓励只有真正独立操作才并发，写操作还要依赖底层事务/锁/幂等。

### Q93. MessageBus 的 publish 与 publish_event 有什么区别？

current MessageBus 同时支持 local typed subscriber 与 Channel outbound queue。

- publish：await 本地 subscriber，适合 runtime state fan-out
- publish_event：把 typed event 转成 routed outbound delivery

本地状态变化不必等待网络发送。

### Q94. TurnDelivery 为什么独立出来？
...
这样 AgentRunner 不需要知道 Telegram/Feishu/WebSocket 的发送细节。

### Q95. MCPProvider 为什么是 application-owned infrastructure？

MCP Connection 的生命周期属于 CLI/Gateway/SDK Composition Root：
...
如果 AgentLoop 自己拥有它，多个入口会重复 lifecycle logic。

### Q96. Skills Progressive Loading 如何实现？

SkillsLoader 先构建 name/description/path/availability summary。

需要时：

- Agent 读取完整 SKILL.md
- 或用户用 $skill-name 显式激活
- always Skill 直接注入

这样减少 Context Bloat。

### Q97. Agent Plugin 为什么需要 Fingerprint？

Installed Plugin 之后可能被文件替换。

Fingerprint 让系统知道“当前 Package Content 是否仍然等于用户此前启用的版本”，避免静默继承信任。

### Q98. Session 与 Long-term Memory 为什么分开？

Session 需要完整 replay、Tool State、Provider State 等 Runtime 信息。

Long-term Memory 只保留跨 Session 稳定信息。

把两者混在一个 MEMORY.md 中会导致：

- Replay 不完整
- Context 过长
- 状态语义混乱

### Q99. 如何测试 Nanobot 的一个 Source Change？

先按变化面选最小 Test：

- Provider → Provider unit/mocked API
- Channel → Channel tests + Gateway startup
- Tool → Tool unit + Agent run path
- WebUI → frontend test/build + Browser flow
- Docs → command/config/link accuracy

用户可见行为最好再走一次真正 public surface。

### Q100. 如果让你改 Nanobot，你会优先加什么？

一个高质量回答不是“再加 10 个 Tool”。

可以选择：

- Agent Trace / Eval Dashboard
- Tool Permission Review
- Retrieval Plugin
- Better Failure Taxonomy
- Session/Memory Visualization
- Provider Cost/Latency Observability

并解释为什么该能力符合现有 Registry/Plugin/Event Extension Point，而不是硬改 Core。
...
## 十一、2026 工程补充题（Q135-Q150）

### Q135. Agent Backend 与普通 CRUD Backend 最大区别是什么？

普通 CRUD 主要面对确定性请求；Agent Backend 还必须处理：
...
### Q136. Agent API 为什么适合 SSE / WebSocket？

LLM/Tool Task 往往耗时数秒到数分钟。Streaming 可以：

- 降低首 Token 等待感
- 展示 Tool Progress
- 支持 Cancellation
- 减少长 HTTP Request 的黑盒感

SSE 更适合单向 Server → Client Stream；WebSocket 适合双向长连接与 richer runtime event。

### Q137. 如何设计 Agent Request Idempotency？

对会产生 Side Effect 的 Tool/Job：
...
### Q138. Agent 系统为什么需要 Queue？

长任务不能长期占住 Web Worker。
...
还可提供 retry、backpressure、dead-letter。

### Q139. Redis 在 Agent Backend 中能做什么？

典型用途：

- Session/Job ephemeral state
- Rate Limit
- Cache
- Distributed Lock
- Stream/Queue
...
但长期事实、审计记录仍应放 Durable Store。

### Q140. RAG 中 Vector Search 与 BM25 为什么经常组合？

Vector 擅长语义相似，BM25 擅长 exact term/identifier。
...
### Q141. Reranker 为什么放在 Candidate Retrieval 之后？

Cross-encoder/LLM Rerank 成本高。
...
### Q142. Retrieval Recall 高但最终 Answer 错，怎么排查？

依次看：

1. Relevant Evidence 是否进入最终 Context
2. Rerank 是否把关键 Chunk 排后
3. Prompt 是否要求 Grounding
4. Context 是否被截断
5. Model Generation 是否误读
6. Citation Renderer 是否关联错

不要直接把问题归因“模型不行”。

### Q143. 如何做 Citation Correctness Eval？

对每个 Claim-Citation Pair：
...
可人工标注一小批 Gold Set，再自动回归。

### Q144. MCP Tool 应该返回自然语言还是 Structured JSON？

优先 Structured Data + 必要可读文本。

Structured Output 更适合：
...
- overall Agent budget

并返回可分类错误，让 Agent 决定 retry、fallback 或 abort。

### Q146. Tool Retry 为什么必须考虑幂等？

GET-like Read Tool 通常可 retry。

Write Tool 如果没有 idempotency key，retry 可能造成重复 Side Effect。

所以 Retry Policy 不能只看 Exception Type。

### Q147. Agent Observability 至少记录什么？

- request/turn/session id
- model/provider
...
敏感 Prompt/Secret 要按 Log Policy 隐藏。

### Q148. Java 后端背景如何迁移到 Agent 开发？
...
Agent 项目同样需要成熟 Backend Engineering。

### Q149. 第一段 Agent 实习项目最应该展示什么？
...
例如不只说“Nanobot 有 Session Lock”，而是说明为什么 per-session FIFO、pending injection、global concurrency 是不同问题。
