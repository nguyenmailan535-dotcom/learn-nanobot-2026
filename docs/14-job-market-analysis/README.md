...
**薪资说明**：不同城市、公司层级、届别与岗位方向差异很大；投递前按当周同类 JD 单独核验，不在本仓库维护固定区间。
...
**为什么重要**：Python 在 LLM、RAG、Agent 与 MCP 生态中非常常见，Agent Runtime 也经常大量使用 async/await；但平台/后端岗位同样可能以 Java、Go 为主，应以目标 JD 为准。
...
**为什么重要**：MCP 已成为 Agent 工具连接生态中值得重点掌握的开放协议之一；对 Agent/AI 平台岗位，理解 Host/Client/Server、Tool Schema、Transport、Lifecycle 与 Security 会形成明显的工程区分度。
...
1. **Agent 正在工程化**：只会 Prompt/API 已不足以代表完整 Agent 开发能力。
2. **后端基本盘仍重要**：HTTP、数据库、缓存、并发、异步任务、Linux、Docker、可观测性会直接出现在 Agent 系统里。
3. **MCP / Tool Calling 是有价值的能力层**：重点是协议、生命周期、权限和错误治理，而不是只会接一个 Demo Server。
4. **RAG 要可评测**：Hybrid Retrieval、Rerank、Grounding、Citation、Eval 和 Bad Case 比“接了向量库”更能体现工程深度。
5. **项目要形成闭环**：源码理解 → Capability（MCP/Tool）→ ResearchPilot → Eval → Deploy → Interview Story。
6. **所有市场判断回到真实 JD**：公司组织、薪酬、岗位数量和技术栈变化快，本章不把未经持续核验的数字当结论。
